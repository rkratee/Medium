...
export const BlogCard = ({
    ...
    <div className="p-4 border-b border-slate-200 pb-4 w-screen max-w-screen-sm cursor-pointer">
        <div className="flex">
            ...
            <div className="pl-2 font-thin font-slate-500"> {publishedDate} </div>
        </div>
        <div className="text-xl font-semibold pt-2">{title}</div>
        <div className="text-md font-thin">{content.slice(0, 100) + "..."}</div>
        <div className="text-slate-500 text-sm font-thin">{`${Math.ceil(content.length / 100)} minute(s)`}</div>
    </div>
    </Link>
}
...
export function Avatar({name, size= "small"}: {name: string, size: "small" | "big"}){
    ...
    <span className={`${size === "small" ? "text-sm": "text-md"}font-thin text-xs text-gray-100 dark:text-gray-300`}>{name[0]}</span>
</div>
}